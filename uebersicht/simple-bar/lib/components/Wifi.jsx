...
const toggleWifi = (isActive) => {
    if (isActive) {
        ...
        run(
            `osascript -e 'tell app "System Events" to display notification "Disabling wifi..." with title "simple-bar"'`
        );
    } else {
        run(`networksetup -setairportpower en0 on`).then(refreshData);
        run(
            `osascript -e 'tell app "System Events" to display notification "Enabling wifi..." with title "simple-bar"'`
        );
    }
};
